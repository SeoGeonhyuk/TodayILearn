...
추가로 프로그래머스 레벨 2 정렬 한 문제를 풀었다.
# 2023.07.26 ~ 08.02
프록시와 로드밸런서의 역할이 무엇인지, 웹에서 쓰이는 포트는 80번 포트 한개밖에 없는데 어떻게 하나의 포트만으로 수많은 웹사이트에 대한 정보교환을 보장하는지 배웠다.<br>
여러 클라이언트들의 요청은 프록시에 의해서 한데 모아져 차례대로 80번 포트로 보내지고 이 많은 정보를 서버의 로드밸런서가 정보를 각각의 CPU에 분산시켜 정보가 한데 모이지 않고 나눠져서 처리되게 하여 서버의 부담을 줄여준다. <br> 이렇게 각 부분에서 처리된 정보들은 로드밸런서에 의해 합쳐치고 다시 역으로 전송되어 각각의 클라이언트들에게 정보가 보내지게 된다.<br>
프록시를 사용하게 될 경우, 캐시처럼 기존에 클라이언트들이 요청했던 정보들을 서버에서 받아와 저장해두어 나중에 똑같은 정보를 클라이언트가 요청했을 경우 프록시에서 직접적으로 바로 보내줄 수 있게 하여, 서버에 대한 부담을 줄일 수 있다.<br>
또한 데이터베이스가 무엇인지, MySql, nodejs를 공부하였다.<br>
프로젝트 진행 중이다.<br>
